...
import { mapConnectors } from '../helpers/map-connectors'
import * as dc from '../helpers/draw-connector'
import { region } from '../helpers/data'
import { mapRound } from '../helpers/map-rounds'
    ...
    teamMidpoints
      ...
      .forEach((round, index, arr) => {
        round.matches.forEach((match) => {
          ...
          if (match.team3.start) {
            dc.drawConnector(match.team3.start, match.team3.end, radius, color, linewidth, round.showSConnector, this.state.context)
          }
          if (match.team4.start) {
            dc.drawConnector(match.team4.start, match.team4.end, radius, color, linewidth, round.showSConnector, this.state.context)
          }
        }, this)
      }, this);   
  }
